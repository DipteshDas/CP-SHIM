...
def run(data_path="../data/friedman2_continuous/"):
    ...
    for i in range(len(sample_sizes)):
        ...
        #
        _, cpl_ss, r2_ss = call_method(dir_ms, dir_cv, n_sample=3, name="shim", split=True, ord=4)
        _, cpl_sf, r2_sf = call_method(dir_ms, dir_cv, n_sample=3, name="shim", split=False, ord=4)
    ...
    np.savez(stat_path, data_mlp=data_mlp, data_rf=data_rf, data_lf=data_lf, data_ls=data_ls, data_sf=data_sf, data_ss=data_ss, sample_sizes=sample_sizes)

    ylim = (0, 5)
    xlabel, ylabel, fname = "Sample size", "CI length", "../results/friedman2_continuous/figures/cpl_friedman2_continuous_50_100_200.pdf"
