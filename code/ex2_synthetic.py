...
def plot(data_mlp, data_rf, data_lf, data_ls, data_sf, data_ss, n_samples, xlabel, ylabel, fname, ylim):
    ...
    bp_mlp = plt.boxplot(data_mlp, patch_artist=True, positions=[3, 12, 21], labels=no_labels,
    ...
    ax.legend([bp_mlp["boxes"][0], bp_rf["boxes"][0], bp_ls["boxes"][0], bp_lf["boxes"][0], bp_ss["boxes"][0],
               bp_sf["boxes"][0]], ['mlp', 'rf', 'lasso_s', 'lasso_f', 'shim3_s', 'shim3_f'], loc='upper right',
              prop={'size': 16}, ncol=3)
...
def run():
    ...
    for i in range(len(sample_sizes)):
        ...
        _, cpl_mlp, r2_mlp = call_method(dir_ms, dir_cv, n_sample=15, name="mlp")
        _, cpl_rf, r2_rf = call_method(dir_ms, dir_cv, n_sample=15, name="rf")

        _, cpl_ls, r2_ls = call_method(dir_ms, dir_cv, n_sample=15, name="shim", split=True, ord=1)
        _, cpl_lf, r2_lf = call_method(dir_ms, dir_cv, n_sample=15, name="shim", split=False, ord=1)

        _, cpl_ss, r2_ss = call_method(dir_ms, dir_cv, n_sample=15, name="shim", split=True, ord=3)
        _, cpl_sf, r2_sf = call_method(dir_ms, dir_cv, n_sample=15, name="shim", split=False, ord=3)
    ...
    stat_path = "../results/synthetic/stat_syn_m_10_n_100_150_200_15_samples.npz"
    np.savez(stat_path, data_mlp=data_mlp, data_rf=data_rf, data_lf=data_lf, data_ls=data_ls, data_sf=data_sf, data_ss=data_ss, r2_mlps=r2_mlps, r2_rfs=r2_rfs, r2_lfs=r2_lfs, r2_lss=r2_lss, r2_sfs=r2_sfs, r2_sss=r2_sss, sample_sizes=sample_sizes)

    ylim = (1, 4.0)
    xlabel, ylabel, fname = "Sample size", "CI length", "../results/synthetic/figures/cpl_m_10_n_100_150_200_15_samples.pdf"
    plot(data_mlp, data_rf, data_lf, data_ls, data_sf, data_ss, sample_sizes, xlabel, ylabel, fname, ylim)

    ylim = (0, +1.1)
    xlabel, ylabel, fname = "Sample size", "r2 score", "../results/synthetic/figures/r2_m_10_n_100_150_200_15_samples.pdf"
